...
; aligned at a 32-bit boundary. The signature is in its own section so the header
...
_start:
    ...
    ; kernel is entered. It's best to minimize the early environment where crucial
    ; features are offline. Note that the processor is not fully initialized yet.
    ; Features such as floating-point instructions and instruction set extensions are
    ...
    ; pushed a multiple of 16 bytes to the stack since (pushed 0 bytes so far),
